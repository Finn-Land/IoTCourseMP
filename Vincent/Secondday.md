...
When the code is run, and we go to the webpage (in our case it was: **192.168.12.164**) we see the following webpage:
...
void loop() {
  ...
  if ((wifiMulti.run() == WL_CONNECTED)) {
    ...
    http.begin("http://192.168.12.161/on"); //HTTP
  ...
  // configure traged server and url
  //http.begin("https://www.howsmyssl.com/a/check", ca); //HTTPS
  http.begin("http://192.168.12.161/off"); //HTTP
